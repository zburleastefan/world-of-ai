...
import defaultUserImg from './../public/defaultUser.svg';
import LoadingDots from "./LoadingDots";
import { rapidApiChatGPT } from "../utils/rapidApiChatGPT";
...
function ChatPage() {
    ...
    const [messages, loading, error] = useCollection(authContext && query(
        collection(db, "users", authContext?.currentUser?.email!, 'chats', authContext?.currentUser?.uid!, 'messages'),
    ...
    const sendMessage = async (e: FormEvent<HTMLFormElement>) => {
        ...
        await fetch("/api/dbPosts", {
            ...
            body: JSON.stringify({
                ...
                authorEmail: authContext?.currentUser?.email!,
                email: authContext?.currentUser?.email,
                avatar: authContext?.currentUser?.photoURL || defaultUserImg,
                prompt: userInput, 
        ...
        // asking GPT 
        const gptResponse = await rapidApiChatGPT(userInput).then(async (response) => {
    ...
    if (loading) return (
        <div className="flex-1 overflow-y-auto justify-center items-center overflow-x-hidden">
    ...
    if (error) return (
        <div className="flex-1 overflow-y-auto  justify-center items-center overflow-x-clip">
            <div className="flex flex-col justify-center items-center h-screen">
                <ErrorIcon className="h-15 w-15 mx-auto mt-6 text-red-500"/>
                <p className="text-red-500 text-2xl text-center mt-20">{error.message}</p>
            </div>
        </div>   
    );
                        ...
                        <div ref={heightRef} className="mx-2 md:mx-8">
                            ...
                            { messages?.docs.map((message) => (
                                <li key={message.id} className={`flex p-2 md:px-6 ${message.data().user.name === "ChatGPT" && "justify-start"} ${message.data().user.name != "ChatGPT" && "justify-end"}`}>
                                    <figure className={`relative hover:scale-105 sm:hover:scale-100 md:hover:scale-105 transition duration-1000 ease-in-out rounded-2xl p-6 shadow-xl shadow-slate-900/10 ${message.data().user.name  === "ChatGPT" && "bg-[#476ec2] hover:bg-opacity-90 justify-start"} ${message.data().user.name  != "ChatGPT" && "bg-[#434654] justify-end" } `}>
                                        ...
                                        <figcaption className="relative mt-4 flex flex-col items-center justify-between border-t border-slate-100/50 pt-4 space-y-1">
                                            <div className={`font-display text-base ${message.data().user.name == "ChatGPT" && "text-white/80"} ${message.data().user.name != "ChatGPT" && "text-white/60"} hover:scale-105 transition duration-700 ease-in-out`}>
                                                {message.data().user.name}
                                            </div>
                                            <Image 
                                                src={message.data().user.avatar || defaultUserImg}
                                                alt={'user img'} 
        ...
        <div className={`text-gray-400 text-sm w-full rounded-full`}>            
            <form onSubmit={sendMessage} className="p-2 flex justify-evenly items-center">
                ...
                <input 
                    ...
                    focus:outline-none placeholder-[#476ec2]`}
                ...
                <button            
                    className="bg-[#476ec2] hover:opacity-50 items-end text-end align-end justify-end text-white 
